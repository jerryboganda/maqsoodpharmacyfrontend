...
{#if path === '/tables/simple'}
  <div class="space-y-6 animate-fade-in">
    <div><h1 class="heading-2 text-secondary-900 dark:text-white">{t('tables.simple_table')}</h1><p class="text-body-sm mt-1 text-secondary-500 dark:text-secondary-400">{t('tables.simple_table_desc')}</p></div>
    <div class="card rounded-xl p-0 overflow-hidden"><!-- svelte-ignore a11y_no_noninteractive_tabindex --><div class="overflow-x-auto" tabindex="0" role="region" aria-label={t('tables.simple_table')}><table class="w-full"><thead class="bg-surface-50 dark:bg-surface-900/30"><tr><th class={`${headClass} py-3 px-4`}>{t('tables.column.id')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.name')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.email')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.role')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.status')}</th></tr></thead><tbody class="divide-y divide-surface-200 dark:divide-surface-700">{#each simpleRows as row}<tr class="hover:bg-surface-50 dark:hover:bg-surface-900/20 transition-colors"><td class={cellClass}>{row.id}</td><td class={cellClass}><div class="font-medium text-secondary-900 dark:text-white">{row.name}</div></td><td class={cellClass}>{row.email}</td><td class={cellClass}>{row.role}</td><td class={cellClass}><span class={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${statusPill(row.status)}`}>{t(`tables.status.${row.status.toLowerCase()}`)}</span></td></tr>{/each}</tbody></table></div></div>
  </div>
{:else if path === '/tables/data'}
  <div class="space-y-6 animate-fade-in">
    <div><h1 class="heading-2 text-secondary-900 dark:text-white">{t('tables.data_table')}</h1><p class="text-body-sm mt-1 text-secondary-500 dark:text-secondary-400">{t('tables.data_table_desc')}</p></div>
    <div class="card rounded-xl p-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4"><div class="lg:col-span-2 relative"><Icon icon={Icons.search} className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-secondary-400" /><input bind:value={query} on:input={() => (page = 1)} class={`${inputClass} pl-10`} placeholder={t('tables.search_placeholder')} aria-label={t('tables.search_placeholder')} /></div><div><select id="data-status" aria-label={t('tables.column.status')} bind:value={status} on:change={() => (page = 1)} class={inputClass}><option value="All">{t('tables.status.all')}</option><option value="Active">{t('tables.status.active')}</option><option value="Invited">{t('tables.status.invited')}</option><option value="Suspended">{t('tables.status.suspended')}</option></select></div></div>
      <div class="mt-5 rounded-xl overflow-hidden border border-surface-200 dark:border-surface-700"><!-- svelte-ignore a11y_no_noninteractive_tabindex --><div class="overflow-x-auto" tabindex="0" role="region" aria-label={t('tables.data_table')}><table class="w-full"><thead class="bg-surface-50 dark:bg-surface-900/30"><tr><th class={`${headClass} py-3 px-4`}>{t('tables.column.id')}</th>{#each [['name','tables.column.name'],['email','tables.column.email'],['team','tables.column.team'],['createdAt','tables.column.created']] as item}<th class={`${headClass} py-3 px-4`}><button type="button" class="inline-flex items-center gap-1 hover:text-secondary-700 dark:hover:text-secondary-200" on:click={() => toggleSort(item[0] as SortKey)}>{t(item[1])}<Icon icon={Icons.chevronDown} className={`w-4 h-4 transition-transform ${sort.key === item[0] && sort.direction === 'desc' ? 'rotate-180' : ''}`} /><span class="sr-only">{t('tables.sort_by', { column: t(item[1]) })}{sortLabel(item[0] as SortKey)}</span></button></th>{/each}<th class={`${headClass} py-3 px-4`}>{t('tables.column.status')}</th></tr></thead><tbody class="divide-y divide-surface-200 dark:divide-surface-700">{#each pageRows as row}<tr class="hover:bg-surface-50 dark:hover:bg-surface-900/20 transition-colors"><td class={cellClass}>{row.id}</td><td class={cellClass}><div class="font-medium text-secondary-900 dark:text-white">{row.name}</div></td><td class={cellClass}>{row.email}</td><td class={cellClass}>{row.team}</td><td class={cellClass}>{row.createdAt}</td><td class={cellClass}><span class={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${statusPill(row.status)}`}>{t(`tables.status.${row.status.toLowerCase()}`)}</span></td></tr>{/each}{#if !pageRows.length}<tr><td colspan="6" class="py-10 px-4 text-center text-sm text-secondary-500">{t('tables.no_results')}</td></tr>{/if}</tbody></table></div></div>
      <div class="mt-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3"><p class="text-sm text-secondary-500 dark:text-secondary-400">{t('tables.showing_results', { shown: pageRows.length, total: sorted.length })}</p><div class="flex items-center gap-2"><button type="button" on:click={() => (page = Math.max(1, page - 1))} disabled={page <= 1} class="px-3 py-2 rounded-lg border border-surface-200 dark:border-surface-700 text-sm text-secondary-700 dark:text-secondary-300 hover:bg-surface-50 dark:hover:bg-surface-800 disabled:opacity-50 disabled:cursor-not-allowed">{t('tables.prev')}</button><span class="px-3 py-2 text-sm text-secondary-700 dark:text-secondary-300">{t('tables.page_of', { page: Math.min(page, totalPages), totalPages })}</span><button type="button" on:click={() => (page = Math.min(totalPages, page + 1))} disabled={page >= totalPages} class="px-3 py-2 rounded-lg border border-surface-200 dark:border-surface-700 text-sm text-secondary-700 dark:text-secondary-300 hover:bg-surface-50 dark:hover:bg-surface-800 disabled:opacity-50 disabled:cursor-not-allowed">{t('tables.next')}</button></div></div>
    </div>
  </div>
{:else}
  <div class="space-y-6"><div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4"><div><h1 class="heading-2 text-secondary-900 dark:text-white">{t('tables.crud_table')}</h1><p class="text-body-sm mt-1 text-secondary-500 dark:text-secondary-400">{t('tables.crud_table_desc')}</p></div><button type="button" on:click={openCreate} class="inline-flex items-center gap-2 px-4 py-2.5 bg-theme-primary hover:bg-theme-primary/90 text-white rounded-xl font-medium transition-colors"><Icon icon={Icons.plus} className="w-[18px] h-[18px]" width={18} height={18} />{t('tables.add_row')}</button></div><div class="card rounded-xl p-6"><div class="relative max-w-xl"><Icon icon={Icons.search} className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-secondary-400" /><input bind:value={query} class={'pl-10 ' + inputClass} placeholder={t('tables.search_by_fields')} aria-label="Search rows" /></div><div class="mt-5 rounded-xl overflow-hidden border border-surface-200 dark:border-surface-700"><!-- svelte-ignore a11y_no_noninteractive_tabindex --><div class="overflow-x-auto" tabindex="0" role="region" aria-label={t('tables.crud_table')}><table class="w-full"><thead class="bg-surface-50 dark:bg-surface-900/30"><tr><th class={`${headClass} py-3 px-4`}>{t('tables.column.id')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.name')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.email')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.team')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.created')}</th><th class={`${headClass} py-3 px-4`}>{t('tables.column.status')}</th><th class={`${headClass} py-3 px-4 text-right`}>{t('tables.column.actions')}</th></tr></thead><tbody class="divide-y divide-surface-200 dark:divide-surface-700">{#each rows.filter((row) => !query || `${row.id} ${row.name} ${row.email} ${row.team}`.toLowerCase().includes(query.toLowerCase())) as row}<tr class="hover:bg-surface-50 dark:hover:bg-surface-900/20 transition-colors"><td class={cellClass}>{row.id}</td><td class={cellClass}><div class="font-medium text-secondary-900 dark:text-white">{row.name}</div></td><td class={cellClass}>{row.email}</td><td class={cellClass}>{row.team}</td><td class={cellClass}>{row.createdAt}</td><td class={cellClass}><span class={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${statusPill(row.status)}`}>{row.status}</span></td><td class={`${cellClass} text-right`}><div class="inline-flex items-center gap-1"><button type="button" on:click={() => openEdit(row)} class="p-2 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors" aria-label={`Edit ${row.name}`}><Icon icon={Icons.edit} width={18} height={18} className="text-secondary-600 dark:text-secondary-300" /></button><button type="button" on:click={() => requestDelete(row.id)} class="p-2 rounded-lg hover:bg-danger-50 dark:hover:bg-danger-900/20 transition-colors" aria-label={`Delete ${row.name}`}><Icon icon={Icons.trash} width={18} height={18} className="text-danger-600" /></button></div></td></tr>{/each}{#if !rows.filter((row) => !query || `${row.id} ${row.name} ${row.email} ${row.team}`.toLowerCase().includes(query.toLowerCase())).length}<tr><td colspan="7" class="py-10 px-4 text-center text-sm text-secondary-500 dark:text-secondary-400">{t('tables.no_results')}</td></tr>{/if}</tbody></table></div></div></div></div>
{/if}
