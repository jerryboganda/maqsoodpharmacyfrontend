    ...
    <form on:submit|preventDefault={submitForm}>
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
          <div class="card rounded-xl p-6"><h3 class="text-lg font-semibold text-secondary-900 dark:text-white mb-4">{t('ecommerce.category')}</h3><select id="product-category" aria-label={t('ecommerce.category')} bind:value={formData.category} class="w-full px-4 py-2.5 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl text-sm">{#each categories as item}<option value={item}>{item}</option>{/each}</select></div>
          ...
          <div class="card rounded-xl p-6"><h3 class="text-lg font-semibold text-secondary-900 dark:text-white mb-4">{t('ecommerce.tags')}</h3><input id="product-tags" aria-label={t('ecommerce.tags')} bind:value={formData.tags} placeholder={t('ecommerce.tags_placeholder')} class="w-full px-4 py-2.5 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl text-sm" /><p class="mt-2 text-xs text-secondary-400">{t('common.separate_with_commas')}</p></div>
