...
<div class="space-y-6 animate-fade-in">
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h1 class="heading-2 text-secondary-900 dark:text-white">{t('ecommerce.products_title')}</h1><p class="text-body-sm text-secondary-500 dark:text-secondary-400 mt-1">{t('ecommerce.products_description')}</p></div><div class="flex items-center gap-3"><a href="/app/ecommerce/products/create" class="flex items-center gap-2 px-4 py-2.5 bg-theme-primary text-white rounded-xl text-sm font-medium"><Icon icon={Icons.plus} width={16} />{t('ecommerce.add_product')}</a></div></div>
  ...
  {:else}
    <div class="space-y-4">{#each sorted as product}<article class="card rounded-xl p-5 flex gap-5"><a href={`/app/ecommerce/products/${product.id}`} class="flex-shrink-0"><img src={product.thumbnail} alt={product.name} class="w-32 h-32 object-cover rounded-lg" /></a><div class="flex-1 min-w-0"><div class="flex items-start justify-between mb-2"><div class="flex-1"><a href={`/app/ecommerce/products/${product.id}`}><h2 class="text-lg font-semibold text-secondary-900 dark:text-white hover:text-theme-primary transition-colors">{product.name}</h2></a><div class="flex items-center gap-3 mt-1 text-sm text-secondary-500 dark:text-secondary-400"><span>{product.brand}</span><span>•</span><span>{product.category}</span><span>•</span><span>{t('ecommerce.sku')}: {product.sku}</span></div></div><a href={`/app/ecommerce/products/${product.id}/edit`} class="px-4 py-2 bg-surface-100 dark:bg-surface-800 text-secondary-900 dark:text-white rounded-lg hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors text-sm">{t('common.edit')}</a></div><p class="text-sm text-secondary-600 dark:text-secondary-400 mb-3 line-clamp-2">{product.description}</p><div class="flex items-center justify-between"><div class="flex items-center gap-4"><div class="flex items-center gap-1"><Icon icon={Icons.star} width={16} height={16} className="text-yellow-500 fill-yellow-500" /><span class="text-sm font-medium text-secondary-900 dark:text-white">{product.rating}</span><span class="text-sm text-secondary-400">({t('ecommerce.reviews_count', { count: product.reviews })})</span></div><div class="text-sm"><span class="text-secondary-500 dark:text-secondary-400">{t('ecommerce.stock')}: </span><span class={'font-medium ' + stockClass(product)}>{product.stock}</span></div>{#if product.sold}<div class="text-sm text-secondary-500 dark:text-secondary-400">{t('ecommerce.sold')}: {product.sold}</div>{/if}</div><div class="flex items-center gap-4"><span class="text-xl font-semibold text-secondary-900 dark:text-white">${product.price.toFixed(2)}</span><button type="button" class="flex items-center gap-2 px-4 py-2 bg-theme-primary text-white rounded-lg hover:bg-theme-primary-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled={!product.stock} on:click={addToCart}><Icon icon={Icons.shopping} width={16} height={16} />{t('ecommerce.add_to_cart')}</button></div></div></div></article>{/each}</div>
  {/if}
