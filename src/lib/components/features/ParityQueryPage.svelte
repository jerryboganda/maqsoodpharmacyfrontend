...
<svelte:head><title>Query Builder - Adminex</title></svelte:head>
  ...
  <div class="bg-white dark:bg-surface-900 rounded-2xl shadow-lg border border-surface-200 dark:border-surface-700 overflow-hidden">
    ...
    <div class="p-6"><div class="flex items-center justify-between mb-4"><div><h3 class="text-lg font-semibold text-secondary-900 dark:text-white">Filters</h3><p class="text-sm text-secondary-500 dark:text-secondary-400">Define conditions to filter your data</p></div><button type="button" on:click={clearQuery} class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-secondary-600 dark:text-secondary-400 hover:text-danger-500 hover:bg-danger-50 rounded-lg transition-colors"><Icon icon={Icons.refresh} className="w-3.5 h-3.5" />Clear All</button></div>
      <div class="relative rounded-xl border-2 border-dashed p-4 transition-all border-accent-500/50 bg-accent-50/30 dark:bg-accent-900/10"><div class="flex items-center justify-between mb-4"><div class="flex items-center gap-3"><div class="flex items-center bg-surface-100 dark:bg-surface-800 rounded-lg p-1"><button type="button" on:click={() => toggleLogic('AND')} class={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${query.filterGroup.logic === 'AND' ? 'bg-accent-500 text-white shadow-sm' : 'text-secondary-600 dark:text-secondary-400'}`}>AND</button><button type="button" on:click={() => toggleLogic('OR')} class={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${query.filterGroup.logic === 'OR' ? 'bg-primary-500 text-white shadow-sm' : 'text-secondary-600 dark:text-secondary-400'}`}>OR</button></div><span class="text-xs text-secondary-500 dark:text-secondary-400">{query.filterGroup.filters.length} filter{query.filterGroup.filters.length !== 1 ? 's' : ''}</span></div><div class="flex items-center gap-2"><button type="button" on:click={addFilter} class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-accent-600 bg-accent-50 dark:bg-accent-900/30 rounded-lg hover:bg-accent-100 transition-colors"><Icon icon={Icons.plus} className="w-3.5 h-3.5" />Filter</button><button type="button" on:click={addFilter} class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-primary-600 bg-primary-50 dark:bg-primary-900/30 rounded-lg hover:bg-primary-100 transition-colors"><Icon icon={Icons.plus} className="w-3.5 h-3.5" />Group</button></div></div>
        <div class="space-y-3">{#each filters as filter, index}<div class="relative">{#if index > 0}<div class="absolute -top-3 left-6"><span class="px-2 py-0.5 text-[10px] font-bold rounded bg-accent-100 text-accent-700">{query.filterGroup.logic}</span></div>{/if}<div class="flex items-start gap-3 p-3 bg-white dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-700 shadow-sm group hover:shadow-md transition-shadow"><div class="flex-1 min-w-[160px]"><select value={filter.field} on:change={(event) => changeFilterField(filter.id, eventValue(event))} class="w-full px-3 py-2 text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg hover:border-accent-300 transition-colors"><option value="">Select field...</option>{#each queryFields as field}<option value={field.name}>{field.label}</option>{/each}</select></div><div class="flex-1 min-w-[140px]"><select value={filter.operator} on:change={(event) => updateFilter(filter.id, { operator: eventValue(event) as QueryFilter['operator'] })} class="w-full px-3 py-2 text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg focus:ring-2 focus:ring-accent-500"><option value={filter.operator}>{getOperatorsForType(filter.fieldType)[0]?.label ?? 'Equals'}</option>{#each getOperatorsForType(filter.fieldType) as operator}{#if operator.value !== filter.operator}<option value={operator.value}>{operator.label}</option>{/if}{/each}</select></div><div class="flex-1 min-w-[140px]"><input value={String(filter.value ?? '')} type={filter.fieldType === 'number' ? 'number' : filter.fieldType === 'date' ? 'date' : 'text'} on:input={(event) => updateFilter(filter.id, { value: filter.fieldType === 'number' ? Number(eventValue(event)) : eventValue(event) })} placeholder="Value..." class="w-full px-3 py-2 text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg focus:ring-2 focus:ring-accent-500" /></div><button type="button" aria-label="Remove filter" on:click={() => removeFilter(filter.id)} class="p-2 text-secondary-400 hover:text-danger-500 hover:bg-danger-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all"><Icon icon={Icons.trash} className="w-4 h-4" /></button></div></div>{/each}</div>
      </div>
    </div>
    ...
    {#if showExport}<div class="px-6 py-4 border-t border-surface-200 dark:border-surface-700"><div class="flex items-center justify-between mb-3"><h4 class="text-sm font-semibold text-secondary-900 dark:text-white">Query Export</h4><select bind:value={exportFormat} class="input-theme w-auto"><option value="sql">SQL</option><option value="json">JSON</option><option value="mongodb">MongoDB</option><option value="graphql">GraphQL</option></select></div><pre class="p-4 bg-surface-900 text-sm text-green-400 rounded-xl overflow-x-auto font-mono">{exportedQuery}</pre></div>{/if}
  </div>
